...
def nms(boxes, scores, overlap=(0.5, 0.2), top_k=200):
    ...
    while idx_f.numel() > 0:
        ...
        ######################################添加代码###################################
        idx_f = torch.autograd.Variable(idx_f, requires_grad=False)
        idx_f = idx_f.data
        ...
        area_f = torch.autograd.Variable(area_f, requires_grad=False)
        area_f = area_f.data
        idx_f = torch.autograd.Variable(idx_f, requires_grad=False)
        idx_f = idx_f.data
        ########################################################
    ...
    while idx_s.numel() > 0:
        ...
        #####################添加代码################################
        # 否者出错RuntimeError: index_select(): functions with out=... arguments don't support automatic differentiation, but one of the arguments requires grad.
        idx_s = torch.autograd.Variable(idx_s, requires_grad=False)
        idx_s = idx_s.data
        x1 = torch.autograd.Variable(x1, requires_grad=False)
        x1 = x1.data
        y1 = torch.autograd.Variable(y1, requires_grad=False)
        ...
        y2 = torch.autograd.Variable(y2, requires_grad=False)
        y2 = y2.data
        x3 = torch.autograd.Variable(x3, requires_grad=False)
        x3 = x3.data
        y3 = torch.autograd.Variable(y3, requires_grad=False)
        y3 = y3.data
        x4 = torch.autograd.Variable(x4, requires_grad=False)
        x4 = x4.data
        y4 = torch.autograd.Variable(y4, requires_grad=False)
        y4 = y4.data
        ###########################################################
