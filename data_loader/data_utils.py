...
def unshrink_offset(poly,ratio):
    area = cv2.contourArea(poly)#计算轮廓面积
...
def generate_rbox(im_size, text_polys, text_tags, training_mask, shrink_ratio):
    ...
    for i, (poly, tag) in enumerate(zip(text_polys, text_tags)):
        try:
            ...
            shrinked_poly = np.array(pco.Execute(-D))#收缩
            # dilated_poly = np.array(pco.Execute(D))
            dilated_poly = np.array(pco.Execute(0))#膨胀
            cv2.fillPoly(G_s, shrinked_poly, 1)#绘制多边形并填充 1
            cv2.fillPoly(G_d, dilated_poly, 1)#
            if not tag:
                cv2.fillPoly(training_mask, shrinked_poly, 0)#用0填充training_mask的shrinked_ploy部分
        except:
            print(poly)
